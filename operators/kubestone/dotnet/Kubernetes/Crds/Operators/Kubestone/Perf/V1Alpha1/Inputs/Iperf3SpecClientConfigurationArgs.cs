// *** WARNING: this file was generated by crd2pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Kubernetes.Types.Inputs.Perf.V1Alpha1
{

    /// <summary>
    /// ClientConfiguration contains the configuration of the iperf3 client
    /// </summary>
    public class Iperf3SpecClientConfigurationArgs : Pulumi.ResourceArgs
    {
        /// <summary>
        /// CmdLineArgs are appended to the predefined iperf3 parameters
        /// </summary>
        [Input("cmdLineArgs")]
        public Input<string>? CmdLineArgs { get; set; }

        /// <summary>
        /// HostNetwork requested for the iperf3 pod, if enabled the hosts network namespace is used. Default to false.
        /// </summary>
        [Input("hostNetwork")]
        public Input<bool>? HostNetwork { get; set; }

        [Input("podLabels")]
        private InputMap<string>? _podLabels;

        /// <summary>
        /// PodLabels are added to the pod as labels.
        /// </summary>
        public InputMap<string> PodLabels
        {
            get => _podLabels ?? (_podLabels = new InputMap<string>());
            set => _podLabels = value;
        }

        /// <summary>
        /// PodScheduling contains options to determine which node the pod should be scheduled on
        /// </summary>
        [Input("podScheduling")]
        public Input<Pulumi.Kubernetes.Types.Inputs.Perf.V1Alpha1.Iperf3SpecClientConfigurationPodSchedulingArgs>? PodScheduling { get; set; }

        /// <summary>
        /// Resources required by the benchmark pod container More info: https://kubernetes.io/docs/concepts/configuration/manage-compute-resources-container/
        /// </summary>
        [Input("resources")]
        public Input<Pulumi.Kubernetes.Types.Inputs.Perf.V1Alpha1.Iperf3SpecClientConfigurationResourcesArgs>? Resources { get; set; }

        public Iperf3SpecClientConfigurationArgs()
        {
        }
    }
}
