// *** WARNING: this file was generated by crd2pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";

import {ObjectMeta} from "../meta/v1";

export namespace redhatcop {
    export namespace v1alpha1 {
        /**
         * MustGatherSpec defines the desired state of MustGather
         */
        export interface MustGatherSpec {
            /**
             * The is of the case this must gather will be uploaded to
             */
            caseID: pulumi.Input<string>;
            /**
             * the secret container a username and password field to be used to authenticate with red hat case management systems
             */
            caseManagementAccountSecretRef: pulumi.Input<inputs.redhatcop.v1alpha1.MustGatherSpecCaseManagementAccountSecretRef>;
            /**
             * The list of must gather images to run, optional, it will default to: $DEFAULT_MUST_GATHER_IMAGE
             */
            mustGatherImages?: pulumi.Input<pulumi.Input<string>[]>;
            /**
             * This represents the proxy configuration to be used. If left empty it will default to the cluster-level proxy configuration.
             */
            proxyConfig?: pulumi.Input<inputs.redhatcop.v1alpha1.MustGatherSpecProxyConfig>;
            /**
             * the service account to use to run the must gather job pod, defaults to default  +kubebuilder:default:="{Name:default}"
             */
            serviceAccountRef?: pulumi.Input<inputs.redhatcop.v1alpha1.MustGatherSpecServiceAccountRef>;
        }

        /**
         * the secret container a username and password field to be used to authenticate with red hat case management systems
         */
        export interface MustGatherSpecCaseManagementAccountSecretRef {
            /**
             * Name of the referent. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names TODO: Add other useful fields. apiVersion, kind, uid?
             */
            name?: pulumi.Input<string>;
        }

        /**
         * This represents the proxy configuration to be used. If left empty it will default to the cluster-level proxy configuration.
         */
        export interface MustGatherSpecProxyConfig {
            /**
             * httpProxy is the URL of the proxy for HTTP requests.  Empty means unset and will not result in an env var.
             */
            httpProxy?: pulumi.Input<string>;
            /**
             * httpsProxy is the URL of the proxy for HTTPS requests.  Empty means unset and will not result in an env var.
             */
            httpsProxy?: pulumi.Input<string>;
            /**
             * noProxy is the list of domains for which the proxy should not be used.  Empty means unset and will not result in an env var.
             */
            noProxy?: pulumi.Input<string>;
        }

        /**
         * the service account to use to run the must gather job pod, defaults to default  +kubebuilder:default:="{Name:default}"
         */
        export interface MustGatherSpecServiceAccountRef {
            /**
             * Name of the referent. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names TODO: Add other useful fields. apiVersion, kind, uid?
             */
            name?: pulumi.Input<string>;
        }

        /**
         * MustGatherStatus defines the observed state of MustGather
         */
        export interface MustGatherStatus {
            completed: pulumi.Input<boolean>;
            lastUpdate?: pulumi.Input<string>;
            reason?: pulumi.Input<string>;
            status?: pulumi.Input<string>;
        }
    }
}
